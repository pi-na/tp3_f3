...
\begin{titlepage}
    ...
    \begin{tabular}{ll}
        ...
        \textbf{Integrantes:} & \textit{Nombre Apellido (Legajo)} \\
                              & \textit{Nombre Apellido (Legajo)} \\
                              & \textit{Nombre Apellido (Legajo)} \\
                              & \textit{Nombre Apellido (Legajo)} \\
...
\begin{figure}[h]
    ...
    \includegraphics[width=0.8\textwidth]{foto_montaje_circuito_RLC_serie.jpg}
    \caption{Montaje experimental del circuito RLC serie (reemplazar por la foto real correspondiente).}
\end{figure}
...
\begin{figure}[h]
    ...
    \includegraphics[width=0.8\textwidth]{captura_osciloscopio_resonancia_RLC.png}
    \caption{Captura de pantalla del osciloscopio mostrando la tensión en la resistencia en la frecuencia de resonancia (reemplazar por la imagen correspondiente).}
\end{figure}
...
\begin{figure}[h]
    ...
    \includegraphics[width=0.8\textwidth]{grafico_VR_vs_frecuencia_RLC_serie.pdf}
    \caption{Tensión en la resistencia en función de la frecuencia, mostrando el máximo de resonancia (reemplazar por el gráfico generado a partir de los datos experimentales).}
\end{figure}
...
De la planilla de laboratorio \texttt{MEDICIONES F3 - TP3DATA.csv} se obtuvieron los siguientes valores:
...
\begin{table}[h]
    ...
    \begin{tabular}{lll}
        ...
        Inductancia consignada & \( L_1 \) & \(\approx \SI{3.2e-10}{\henry}\) (según planilla) \\
        Frecuencia de resonancia (planilla) & \( f_{0,\text{planilla}} \) & \(\approx \SI{4.0e3}{\hertz}\) \\
        \bottomrule
    \end{tabular}
    \caption{Parámetros del circuito RLC serie obtenidos de la planilla de laboratorio.}
\end{table}

Como se discutirá más adelante, el valor de \( L_1 \) consignado en la planilla no resulta consistente con la frecuencia de resonancia medida, por lo que se determinó una inductancia efectiva \( L_{\text{exp}} \) a partir de los datos experimentales.

\subsection{Tensión en la resistencia en función de la frecuencia}
En la misma planilla se registraron los valores de la tensión máxima sobre la resistencia \( V_R \) para distintos valores de frecuencia en el entorno de la resonancia. A continuación se presenta una tabla resumen con las mediciones relevantes:
...
Este valor de \( L_{\text{exp}} \) es coherente con un inductor de unas pocas milésimas de henry y, como se mencionó previamente, difiere considerablemente del valor consignado en la planilla (\( \sim 10^{-10}\,\si{\henry} \)), lo que sugiere un error de transcripción o de formato en el registro original.
...
Una estimación más detallada de las incertidumbres requeriría disponer de los errores absolutos de \( R \), \( C \), \( f \) y \( V_R \), información que no se encontraba completamente especificada en la planilla. No obstante, cualitativamente puede afirmarse que la diferencia entre \( Q_{\text{exp}} \) y \( Q_{\text{teo}} \) está dominada por las pérdidas resistivas adicionales del circuito real.
